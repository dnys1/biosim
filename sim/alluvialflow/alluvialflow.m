function h = alluvialflow(data, left_labels, right_labels, chart_title, axes)
...
%    parameters etc.
    ...
    x_left = 0;
    x_right = 1.05;
    axis(axes, [x_left, x_right, 0, y_height]) % Set limits
    axis(axes, 'ij') % origin is top left
    axis(axes, 'off')
    
    % grid minor % DBG
    
    hold(axes, 'on')
    patch(axes, [0 0 1 1], [0 y_height y_height 0], 'w');
    ...
    plot(axes, zeros(2, numel(left_category_points)), [left_category_points; (left_category_points + left_category_sizes)], 'k', 'LineWidth',4);
    ...
    plot(axes, ones(2, numel(right_category_points)), [right_category_points; (right_category_points + right_category_sizes)], 'k', 'LineWidth',4);
    ...
    for k_left = 1:size(data, 1) % for each row
        ...
        patch(axes, 'XData', X, 'YData', Y, 'FaceColor', color, 'FaceAlpha', .4, 'EdgeColor', 'none');
    end % for each row
    
    % Place left labels
    text(axes, zeros(1, size(data, 1)) - 0.01, ...
    ...
    % Place right labels
    text(axes, ones(1, size(data, 2)) + 0.01, ...
